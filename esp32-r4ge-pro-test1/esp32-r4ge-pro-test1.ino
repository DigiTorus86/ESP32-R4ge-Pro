...
{
  Serial.begin(115200);
  Serial.println("ESP32 Badge Pro Test1 I2S"); 
  delay(100);
  ...
  for(int i = 0; i < SAMPLE_SIZE; i++) 
      audio_sample[i] = (4000 * sin((double)i * two_pi_scaled));
...
void playAudio(bool play_right, bool play_left)
{
  ...
  size_t    bytes_out;
	...
	while (audio_playing)
	{
		...
		{
		  temp = (audio_sample[sample_pos + i]); // << 2;
		  temp_msb = temp >> 8;
		  
		  if (play_left) // put sound data into right channel
		  {
		  ...
		  if (play_right) // put sound data into left channel
		  {
